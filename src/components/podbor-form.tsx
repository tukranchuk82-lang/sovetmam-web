  ...
  if (results) {
    return (
      <div ref={topRef} className="px-4 py-5">
        ...
        {/* Почти вся поддержка в базе адресована семьям с детьми и будущим
            родителям. Если в анкете ни того, ни другого — список честно почти
            пуст, и человек должен понимать почему, а не решать, что подбор
            сломался. */}
        {pregnant === false && hasChildren === false && (
          <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3.5 text-sm leading-snug text-amber-900">
            В анкете указано, что детей пока нет и ребёнка вы не ждёте. Почти
            все меры поддержки положены семьям с детьми или будущим родителям,
            поэтому список короткий. Если что-то изменилось — обновите ответы.
          </p>
        )}
