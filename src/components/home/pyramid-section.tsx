...
const N = 7; // число уровней
const VBH = TOP0 + H + TOP0;

// 7 оттенков — плавный переход в той же палитре (#B21F3A → #F5DCE2).
const COLORS = ["#B21F3A", "#BD3F56", "#C85E72", "#D47E8E", "#DF9DAA", "#EABDC6", "#F5DCE2"];

// Подписи по индексу уровня (сверху вниз): вершина → основание.
const LABELS = ["Бизнес", "НКО", "ВУЗ", "От работодателя", "Муниципальные", "Региональные", "Федеральные"];
// Кегль растёт книзу так, чтобы каждое нижнее слово было шире вышестоящего.
const LABEL_SIZE = [9, 12, 13, 14, 17, 19, 22];
// На тёмных верхних полосах — белый текст, на светлых нижних — тёмно-винный.
const LABEL_COLOR = ["#fff", "#fff", "#fff", "#fff", "#7A1528", "#7A1528", "#7A1528"];

// Подсказки по индексу уровня (сверху вниз).
const TOOLTIP: { title: string; desc: string; examples: string[] }[] = [
  {
    title: "От бизнеса",
    desc: "Скидки и льготы семьям с детьми от коммерческих компаний.",
    examples: ["Скидки многодетным в магазинах", "Специальные предложения для семей", "Льготы на услуги и сервисы"],
  },
  {
    title: "НКО и фонды",
            ...
            {LEVELS.map((lvl, i) => (
              <span
                ...
                style={{
                  ...
                  color: LABEL_COLOR[i],
                  // Верхнюю подпись («Бизнес») опускаем ниже, в широкую часть
                  // вершины-треугольника, иначе слово не помещается по ширине.
                  marginTop: i === 0 ? "16px" : undefined,
                }}
              >
                {LABELS[i]}
              </span>
