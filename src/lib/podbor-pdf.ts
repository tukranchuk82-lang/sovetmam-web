...
export function buildPodborPdf(input: {
  ...
  return (async () => {
    ...
    // Регион не указан — в файле только федеральные меры. Об этом честно
    // предупреждаем: иначе человек решит, что в его области ничего нет.
    if (!input.region) {
      text(
        ctx,
        "В анкете не указан регион, поэтому здесь только федеральные меры — " +
          "они действуют по всей стране. Укажите регион в анкете, и к ним " +
          "добавятся меры вашей области: губернаторские выплаты, региональный " +
          "материнский капитал, льготы на ЖКУ и проезд.",
        { size: 9.5, bold: true, color: BORDO, gap: 8 },
      );
    }
    rule(ctx);
